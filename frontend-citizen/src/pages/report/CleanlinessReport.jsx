        ...
        async (pos) => {
          ...
          try {
            ...
            if (data.display_name) {
              setLocation(data.display_name); // ✅ human-readable address
            } else {
              setLocation(coords); // fallback to raw coordinates
            }
          } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await reportIssue(
        "cleanliness", // case-insensitive match with DB
        description,
        name,
        finalLocation,
        photo
      ...
      <aside className="w-64 bg-gradient-to-b from-cyan-900 to-purple-400 text-white p-6 flex flex-col justify-between">
        <div>
          ...
          <nav className="space-y-6">
            ...
            <button onClick={() => navigate("/track/1")} className="hover:text-yellow-300">
              Track Progress
            </button>
